...
g_period integer := 12;
L1_threshold integer := 24;
L0_threshold integer := 168;
...
if db_rolec=1 or ARCHIVE_LOG_MODE='NOARCHIVELOG' THEN
 OUTPUT_MESSAGE := 'NO';
else
...
05/31/2017 -- Ravi - Modified logic in db_rolec and Added != 'COMPLETED' or L11 != 'COMPLETED' then
...
07/11/2019 -- Rommell/MikeP - Modifed to work for GBUCS2(n1vgbucrdbp0010.n1.int.oracleindustry.com). Its a 4 node RAC server. The main rman jobs on node 2. 
			-- Removed the L1 check on the code and changed the grace period from 12 hours to 48 hours on L0 and 12 for archived log backup. 
			-- Also removed the OS check since the original code was used for linux only.
*/
